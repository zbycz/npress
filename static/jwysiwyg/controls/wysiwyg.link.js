	...
	*/
	$.wysiwyg.controls.link = {
		init: function (Wysiwyg) {
			...
			formLinkHtml = '<form class="wysiwyg"><fieldset><legend>{legend}</legend>' +
				'<label>Zvolte stránku: <select name="id_page"></select></label>' +
				'<label>nebo napište adresu: <input type="text" name="linkhref" value=""/></label>' +
				'<input type="submit" class="button" value="{submit}"/> ' +
				'<input type="reset" value="{reset}"/></fieldset></form>';
			...
			if ($.fn.dialog) { //fill user data to the form
				...
				var match = /#-p([0-9]+)-#$/.exec(a.href); //TODO it gives absolute url ending with this macro...
				if(match)
					$(selectBox).val( match[1] );
				else
					...
					open: function (ev, ui) {
						$("input:submit", dialog).click(function (e) {
							...
							var url = $('input[name="linkhref"]', dialog).val(),
								id_page = $('select[name="id_page"]', dialog).val(),
							...
							if (a.self) {
								if ("string" === typeof (url)) {
									if (url.length > 0) {
										...
										$(a.self).replaceWith(a.self.innerHTML); //TODO whats this?
									}
								}
							} else {
								if ($.browser.msie) {
