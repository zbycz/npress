...
function tag2npmacro(newContent) {
  ...
  newContent = newContent.replace(/<[^#>]*#-(file-.+?)-#[^>]*>/gi, function(
    tag,
    macro
  ) {
    ...
    if ((matches = /width[^0-9]{1,2}([0-9]+)/.exec(tag))) {
      ...
      if (opts[1] && opts[1].match(/^\d+(x\d+)?$/)) opts[1] = size;
      else opts.splice(1, 0, size);
    }
...
function npmacro2tag(newContent) {
  ...
  newContent = newContent.replace(/#-(file-(\d+)(_[^_#]+)*)-#/gi, function(
    str,
    macro,
    id
  ) {
...
function pageEditForm_jwysiwyg() {
  if (!wysiwygConfig)
    wysiwygConfig = { contentStyle: false, minHtmlHeading: 2 };
...
function live_typing() {
  //live typing (on newly created page)
  var fheading = $("#frmpageEditForm-heading");
  var fname = $("#frmpageEditForm-name");
  var fseoname = $("#frmpageEditForm-seoname");
  ...
  if (window.location.hash == "#newpage") {
    fheading.focus().keyup(function() {
      ...
      if (fname.val() == fheadingPrevVal) fname.val(fheading.val());
...
function ctrl_s_saving() {
  // catching ctrl+s in body and wysiwyg
...
function foldable_helper() {
  ...
  //displayed UL - show (-)
  if ($this.css("display") != "none") span.html("&ndash;");
...
function metalist() {
  ...
  $("#js-showHiddenMeta")
    .show()
    .click(metalist_reflectCheckbox);
...
function make_url(s) {
  ...
  for (var i = 0; i < s.length; i++) {
    s2 +=
      typeof nodiac[s.charAt(i)] != "undefined"
        ? nodiac[s.charAt(i)]
        : s.charAt(i);
  }
