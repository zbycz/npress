...
count(self::$vars)-1;}}define('NETTE',TRUE);define('NETTE_DIR',dirname(__FILE__));define('NETTE_VERSION_ID',20000);define('NETTE_PACKAGE','PHP 5.2');interface
IBarPanel{function
getTab();function
getPanel();}interface
IPresenter{function
...
ObjectMixin::callStatic(__CLASS__,$name,$args);}static
function
...
function
initializePanel(Application$application){Debugger::$blueScreen->addPanel(create_function('$e','extract(NCFix::$vars['.NCFix::uses(array('application'=>$application)).'], EXTR_REFS);
			return $e ? NULL : array(
...
__construct(IDIContainer$context){$this->context=$context;}final
function
...
new
RedirectResponse($url,IHttpResponse::S301_MOVED_PERMANENTLY);}}$params=$request->getParameters();if(!isset($params['callback'])){return;}$params['presenter']=$this;$method=callback($params['callback'])->toReflection();$response=$method->invokeArgs(PresenterComponentReflection::combineArgs($method,$params));if(is_string($response)){$response=array($response,array());}if(is_array($response)){if($response[0]instanceof
...
__construct($baseDir,IDIContainer$context){$this->baseDir=$baseDir;$this->context=$context;}function
...
getPresenterClass(&$name){if(isset($this->cache[$name])){list($class,$name)=$this->cache[$name];return$class;}if(!is_string($name)||!Strings::match($name,"#^[a-zA-Z\x7f-\xff][a-zA-Z0-9\x7f-\xff:]*$#")){throw
new
...
ClassReflection($arg);$params=array();foreach($rc->getProperties(ReflectionProperty::IS_PUBLIC)as$rp){if(!$rp->isStatic()&&$rp->hasAnnotation('persistent')){$params[]=$rp->getName();}}return$params;}function
...
FileTemplate;$presenter=$this->getPresenter(FALSE);$template->onPrepareFilters[]=callback($this,'templatePrepareFilters');$template->registerHelperLoader('TemplateHelpers::loader');$template->control=$template->_control=$this;$template->presenter=$template->_presenter=$presenter;if($presenter
instanceof
...
function
receiveHttpData(){$httpRequest=$this->getHttpRequest();if(strcasecmp($this->getMethod(),$httpRequest->getMethod())){return;}if($httpRequest->isMethod('post')){$data=Arrays::mergeTree($httpRequest->getPost(),$httpRequest->getFiles());}else{$data=$httpRequest->getQuery();}if($tracker=$this->getComponent(self::TRACKER_ID,FALSE)){if(!isset($data[self::TRACKER_ID])||$data[self::TRACKER_ID]!==$tracker->getValue()){return;}}return$data;}function
...
__toString(){try{return$this->getRenderer()->render($this);}catch(Exception$e){if(func_get_args()&&func_get_arg(0)){throw$e;}else{Debugger::toStringException($e);}}}protected
function
...
getParameters(){return$this->params;}function
__toString(){try{return$this->component->link($this->destination,$this->params);}catch(Exception$e){Debugger::toStringException($e);}}}class
Multiplier
extends
...
__construct(IDIContainer$context){$this->context=$context;if($this->invalidLinkMode===NULL){$this->invalidLinkMode=$context->parameters['productionMode']?self::INVALID_LINK_SILENT:self::INVALID_LINK_WARNING;}}final
function
...
changeAction($action){if(is_string($action)&&Strings::match($action,'#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*$#')){$this->action=$action;$this->view=$action;}else{$this->error('Action name is not alphanumeric string.');}}final
function
...
ForwardResponse($destination));}elseif(!is_array($args)){$args=func_get_args();array_shift($args);}$this->createRequest($this,$destination,$args,'forward');$this->sendResponse(new
...
new
InvalidLinkException($e->getMessage());}}if(isset($signal)){$reflection=new
...
PresenterRequest($presenter,PresenterRequest::FORWARD,$args,array(),array());$this->lastCreatedRequestFlag=array('current'=>$current);if($mode==='forward'){return;}$url=$router->constructUrl($this->lastCreatedRequest,$refUrl);if($url===NULL){unset($args[self::ACTION_KEY]);$params=urldecode(http_build_query($args,NULL,', '));throw
new
...
function
getPersistentComponents(){$arg=func_get_arg(0);return(array)ClassReflection::from($arg)->getAnnotation('persistent');}private
function
...
new
BadRequestException("Invalid value for parameter '$name', expected ".gettype($def).".");}}}$res[$i++]=$val;}}return$res;}}class
Cache
extends
Object
implements
...
create_function('','extract(NCFix::$vars['.NCFix::uses(array('cache'=>$cache,'function'=>$function)).'], EXTR_REFS);
...
new
CachingHelper($this,$key);}echo$data;}function
...
addServer($host='localhost',$port=11211,$timeout=1){Debugger::tryError();$this->memcache->addServer($host,$port,TRUE,1,$timeout);if(Debugger::catchError($e)){throw
new
...
instanceof
Closure?$t:array($t,'__invoke');}else{$this->cb=$t;}}else{$this->cb=array($t,$m);}if(is_string($this->cb)&&$a=strrpos($this->cb,'\\')){$this->cb=substr($this->cb,$a+1);}elseif(is_array($this->cb)&&is_string($this->cb[0])&&$a=strrpos($this->cb[0],'\\')){$this->cb[0]=substr($this->cb[0],$a+1);}if(!is_callable($this->cb,TRUE)){throw
new
...
new
MethodReflection($this->cb[0],$this->cb[1]);}else{return
new
...
PHP_SAPI==='cli';}static
function
isProduction(){if(self::$productionMode===NULL){self::$productionMode=Configurator::detectProductionMode();}return
...
function
setContext(IDIContainer$context){if(self::$createdAt){throw
new
...
new
Cache(self::getContext()->cacheStorage,$namespace);}static
function
...
ob_get_clean();}function
__toString(){try{return$this->toString();}catch(Exception$e){Debugger::toStringException($e);}}function
...
load($file){Debugger::tryError();$ini=parse_ini_file($file,TRUE);if(Debugger::catchError($e)){throw
new
...
new
InvalidStateException("Replacing operator is available only for arrays, item '$key' is not array.");}$key=substr($key,0,-1);$val[ConfigHelpers::EXTENDS_KEY]=ConfigHelpers::OVERWRITE;}elseif(preg_match('#^(\S+)\s+'.self::INHERITING_SEPARATOR.'\s+(\S+)$#',$key,$matches)){if(!is_array($val)&&$val!==NULL){throw
new
...
implode("\n\n\n",$classes);}static
function
parseServices(DIContainerBuilder$container,array$config,$namespace=NULL){$services=isset($config['services'])?$config['services']:array();$factories=isset($config['factories'])?$config['factories']:array();if($tmp=array_intersect_key($services,$factories)){$tmp=implode("', '",array_keys($tmp));throw
new
ServiceCreationException("It is not allowed to use services and factories with the same names: '$tmp'.");}$all=$services+$factories;uasort($all,create_function('$a, $b','
			return strcmp(ConfigHelpers::isInheriting($a), ConfigHelpers::isInheriting($b));
		'));foreach($all
as$name=>$def){$shared=array_key_exists($name,$services);$name=($namespace?$namespace.'.':'').$name;if(($parent=ConfigHelpers::takeParent($def))&&$parent!==$name){$container->removeDefinition($name);$definition=$container->addDefinition($name);if($parent!==ConfigHelpers::OVERWRITE){foreach($container->getDefinition($parent)as$k=>$v){$definition->$k=$v;}}}elseif($container->hasDefinition($name)){$definition=$container->getDefinition($name);if($definition->shared!==$shared){throw
new
...
new
ServiceCreationException("Service '$name': ".$e->getMessage());}}}static
function
...
array('appDir'=>isset($trace[1]['file'])?dirname($trace[1]['file']):NULL,'wwwDir'=>isset($_SERVER['SCRIPT_FILENAME'])?dirname($_SERVER['SCRIPT_FILENAME']):NULL,'productionMode'=>self::detectProductionMode(),'environment'=>self::detectProductionMode()?self::PRODUCTION:self::DEVELOPMENT,'consoleMode'=>PHP_SAPI==='cli','container'=>array('class'=>'SystemContainer','parent'=>'DIContainer'));}function
...
function
detectProductionMode($list=NULL){$list=is_string($list)?preg_split('#[,\s]+#',$list):$list;$list[]='127.0.0.1';$list[]='::1';return!in_array(isset($_SERVER['REMOTE_ADDR'])?$_SERVER['REMOTE_ADDR']:php_uname('n'),$list,TRUE);}}class
ConstantsExtension
extends
...
logQuery(Statement$result,array$params=NULL){if($this->disabled){return;}$source=NULL;foreach(debug_backtrace(FALSE)as$row){if(isset($row['file'])&&is_file($row['file'])&&strpos($row['file'],NETTE_DIR.DIRECTORY_SEPARATOR)!==0){if(isset($row['function'])&&strpos($row['function'],'call_user_func')===0)continue;if(isset($row['class'])&&is_subclass_of($row['class'],'\\Nette\\Database\\Connection'))continue;$source=array($row['file'],(int)$row['line']);break;}}$this->totalTime+=$result->getTime();$this->queries[]=array($result->queryString,$params,$result->getTime(),$result->rowCount(),$result->getConnection(),$source);}static
function
renderException($e){if($e
instanceof
PDOException&&isset($e->queryString)){return
array('tab'=>'SQL','panel'=>DatabaseHelpers::dumpSql($e->queryString));}}function
...
as$col){$s.="<td>{$h($col)}</td>";}$s.="</tr>";}$s.="</table>";}if($source){$s.=DebugHelpers::editorLink($source[0],$source[1])->class('nette-DbConnectionPanel-source');}$s.='</td><td>';foreach($params
...
new
FileNotFoundException("Cannot open file '$file'.");}$count=0;$sql='';while(!feof($handle)){$s=fgets($handle);$sql.=$s;if(substr(rtrim($s),-1)===';'){$connection->exec($sql);$sql='';$count++;}}fclose($handle);return$count;}}class
...
Cache($this->cacheStorage,'Nette.Database.'.md5($connection->getDsn()));$this->structure=($tmp=$this->cache->load('structure'))?$tmp:$this->structure;}}function
...
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$cmd=strtoupper(substr(ltrim($sql),0,6));$this->arrayMode=$cmd==='INSERT'||$cmd==='REPLAC'?'values':'assoc';$sql=Strings::replace($sql,'~\'.*?\'|".*?"|\?~s',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
...
function
detectColumnTypes(){if($this->types===NULL){$this->types=array();if($this->connection->getSupplementalDriver()->isSupported(ISupplementalDriver::META)){$col=0;while($meta=$this->getColumnMeta($col++)){if(isset($meta['native_type'])){$this->types[$meta['name']]=DatabaseHelpers::detectType($meta['native_type']);}}}}return$this->types;}function
...
__toString(){try{return(string)$this->getPrimary();}catch(Exception$e){Debugger::toStringException($e);}}function
...
ref($key,$throughColumn=NULL){list($table,$column)=$this->table->getConnection()->getDatabaseReflection()->getBelongsToReference($this->table->getName(),$key);$column=($tmp=$throughColumn)?$tmp:$column;return$this->getReference($table,$column);}function
related($key,$throughColumn=NULL){if(strpos($key,'.')!==FALSE){list($key,$throughColumn)=explode('.',$key);}list($table,$column)=$this->table->getConnection()->getDatabaseReflection()->getHasManyReference($this->table->getName(),$key);return$this->table->getReferencingTable($table,($tmp=$throughColumn)?$tmp:$column,$this[$this->table->getPrimary()]);}function
...
__construct($table,Connection$connection){$this->name=$table;$this->connection=$connection;$this->primary=$connection->getDatabaseReflection()->getPrimary($table);$this->delimitedName=$this->tryDelimite($this->name);$this->delimitedPrimary=$connection->getSupplementalDriver()->delimite($this->primary);}function
...
instanceof
TableSelection){$clone=clone$parameters;if(!$clone->select){$clone->select=array($clone->primary);}if($this->connection->getAttribute(PDO::ATTR_DRIVER_NAME)!=='mysql'){$condition.=' IN ('.$clone->getSql().')';}else{$in=array();foreach($clone
...
TableSelection($this->name,$this->connection);$selection->where=$this->where;$selection->parameters=$this->parameters;$selection->conditions=$this->conditions;$selection->select($function);foreach($selection->fetch()as$val){return$val;}}function
...
preg_replace_callback('#(?<=[\s,<>=]|^)[a-z_][a-z0-9_.]*(?=[\s,<>=]|$)#i',create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('driver'=>$driver)).'], EXTR_REFS);
...
TableRow($data,$this);}else{return
new
...
TableRow?$row[$column]->getPrimary():$row[$column];$keys[$key]=TRUE;}if($referenced!==NULL&&$keys===array_keys($this->rows)){$this->checkReferenceNewKeys=FALSE;return$referenced;}if($keys){$referenced=new
TableSelection($table,$this->connection);$referenced->where($table.'.'.$referenced->primary,array_keys($keys));}else{$referenced=array();}}return$referenced;}function
getReferencingTable($table,$column,$active=NULL){$referencing=new
GroupedTableSelection($table,$this,$column,$active);$referencing->where("$table.$column",array_keys((array)$this->rows));return$referencing;}function
...
extends
TableSelection{private$refTable;private$column;private$delimitedColumn;private$active;function
__construct($name,TableSelection$refTable,$column,$active=NULL){parent::__construct($name,$refTable->connection);$this->refTable=$refTable;$this->column=$column;$this->delimitedColumn=$this->connection->getSupplementalDriver()->delimite($this->column);$this->active=$active;}function
...
TableSelection($this->name,$this->connection);$selection->where=$this->where;$selection->parameters=$this->parameters;$selection->conditions=$this->conditions;$selection->select($function);$selection->select("{$this->name}.{$this->column}");$selection->group("{$this->name}.{$this->column}");foreach($selection
...
Closure&&!$service
instanceof
Callback){$this->registry[$name]=$service;$this->meta[$name]=$meta;return$this;}elseif(!is_string($service)||strpos($service,':')!==FALSE||$service[0]==="\0"){$service=callback($service);}$this->factories[$name]=array($service);$this->registry[$name]=&$this->factories[$name][1];$this->meta[$name]=$meta;return$this;}function
...
new
InvalidArgumentException("Class $class is not instantiable.");}elseif($constructor=$rc->getConstructor()){return$rc->newInstanceArgs(DIHelpers::autowireArguments($constructor,$args,$this));}elseif($args){throw
new
InvalidArgumentException("Unable to pass arguments, class $class has no constructor.");}return
new$class;}function
callMethod($function,array$args=array()){$callback=callback($function);return$callback->invokeArgs(DIHelpers::autowireArguments($callback->toReflection(),$args,$this));}function
expand($s){return
...
function
getMethodName($name,$isService=TRUE){$uname=ucfirst($name);return($isService?'createService':'create').($name===$uname?'_':'').strtr($uname,'.','_');}}class
DIContainerBuilder
extends
Object{const
...
new
InvalidStateException("Class $def->class has not been found.");}$def->class=ClassReflection::from($def->class)->getName();if($def->autowired){foreach(class_parents($def->class)+class_implements($def->class)+array($def->class)as$parent){$this->classes[strtolower($parent)][]=$name;}}}foreach($this->classes
...
as$name=>$def){try{$type=($tmp=$def->class)?$tmp:'object';$methodName=DIContainer::getMethodName($name,$def->shared);if(!PhpHelpers::isIdentifier($methodName)){throw
new
...
new
ServiceCreationException("Service '$name': ".$e->getMessage());}}return$class;}private
function
...
formatStatement(DIStatement$statement,$self=NULL){$entity=$this->normalizeEntity($statement->entity);$arguments=(array)$statement->arguments;if(is_string($entity)&&Strings::contains($entity,'?')){return$this->formatPhp($entity,$arguments,$self);}elseif($service=$this->getServiceName($entity)){if($this->definitions[$service]->shared){if($arguments){throw
new
...
new
InvalidArgumentException("No service of type {$parameter->getClass()->getName()} found");}}else{if($container
instanceof
...
DIStatement($target,$args);return$this;}function
...
__construct($entity,$arguments=NULL){$this->entity=$entity;$this->arguments=$arguments;}}class
FormGroup
extends
...
loadHttpData(){$path=$this->getHtmlName();$path=explode('[',strtr(str_replace(']','',strpos($path,'[')===FALSE?$path.'.x':substr($path,0,-2)),'.','_'));$this->setValue(Arrays::get($this->getForm()->getHttpData(),$path,NULL)!==NULL);}}class
SelectBox
extends
...
getControl(){$control=parent::getControl();if($this->prompt){reset($this->items);$control->data('nette-empty-value',$this->useKeys?key($this->items):current($this->items));}$selected=$this->getValue();$selected=is_array($selected)?array_flip($selected):array($selected=>TRUE);$option=Html::el('option');foreach($this->items
...
instanceof
Html){$dest->add((string)$value2->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$value2=$this->translate((string)$value2);$dest->add((string)$option->value($key2)->selected(isset($selected[$key2]))->setText($value2));}}}return$control;}}class
MultiSelectBox
extends
SelectBox{function
...
sanitize($value){if($this->control->maxlength&&Strings::length($value)>$this->control->maxlength){$value=Strings::substring($value,0,$this->control->maxlength);}return
Strings::trim(strtr($value,"\r\n",'  '));}function
...
Arrays::get($this->files,$args,NULL);}final
function
...
__construct(IHttpRequest$request,IHttpResponse$response){$this->request=$request;$this->response=$response;}function
start(){if(self::$started){return;}$this->configure($this->options);Debugger::tryError();session_start();if(Debugger::catchError($e)&&!session_id()){@session_write_close();throw
new
InvalidStateException('session_start(): '.$e->getMessage(),0,$e);}self::$started=TRUE;unset($_SESSION['__NT'],$_SESSION['__NS'],$_SESSION['__NM']);$nf=&$_SESSION['__NF'];if(empty($nf)){$nf=array('C'=>0);}else{$nf['C']++;}$nfTime=&$nf['Time'];$time=time();if($time-$nfTime>self::REGENERATE_INTERVAL){$nfTime=$time;$this->regenerationNeeded=TRUE;}$browserKey=$this->request->getCookie('nette-browser');if(!$browserKey){$browserKey=Strings::random();}$browserClosed=!isset($nf['B'])||$nf['B']!==$browserKey;$nf['B']=$browserKey;$this->sendCookie();if(isset($nf['META'])){$now=time();foreach($nf['META']as$section=>$metadata){if(is_array($metadata)){foreach($metadata
...
new
InvalidStateException("Unable to set 'session.$key' to value '$value' when session has been started by session.auto_start or session_start().");}if(isset($special[$key])){$key="session_$key";$key($value);}elseif(function_exists('ini_set')){ini_set("session.$key",$value);}elseif(!Framework::$iAmUsingBadHost){throw
new
...
function
sendCookie(){$cookie=$this->getCookieParameters();$this->response->setCookie(session_name(),session_id(),$cookie['lifetime']?$cookie['lifetime']+time():0,$cookie['path'],$cookie['domain'],$cookie['secure'],$cookie['httponly'])->setCookie('nette-browser',$_SESSION['__NF']['B'],HttpResponse::BROWSER,$cookie['path'],$cookie['domain']);}}final
class
SessionSection
extends
Object
...
as$this->position=>$token){if($token->type===LatteToken::TEXT){$this->output.=$token->text;}elseif($token->type===LatteToken::MACRO){$isRightmost=!isset($tokens[$this->position+1])||substr($tokens[$this->position+1]->text,0,1)==="\n";$this->writeMacro($token->name,$token->value,$token->modifiers,$isRightmost);}elseif($token->type===LatteToken::TAG_BEGIN){$this->processTagBegin($token);}elseif($token->type===LatteToken::TAG_END){$this->processTagEnd($token);}elseif($token->type===LatteToken::ATTRIBUTE){$this->processAttribute($token);}}}catch(CompileException$e){$e->sourceLine=$token->line;throw$e;}foreach($this->htmlNodes
...
MacroNode
extends
Object{public$macro;public$name;public$isEmpty=FALSE;public$args;public$modifiers;public$closing=FALSE;public$tokenizer;public$parentNode;public$openingCode;public$closingCode;public$attrCode;public$content;public$data;public$htmlNode;public$saved;function
...
function
initRuntime($template,$global){if(!empty($global->caches)){end($global->caches)->dependencies[Cache::FILES][]=$template->getFile();}}static
function
createCache(ICacheStorage$cacheStorage,$key,&$parents,$args=NULL){if($args){if(array_key_exists('if',$args)&&!$args['if']){return$parents[]=(object)NULL;}$key=array_merge(array($key),array_intersect_key($args,range(0,count($args))));}if($parents){end($parents)->dependencies[Cache::ITEMS][]=$key;}$cache=new
...
macroInclude(MacroNode$node,$writer){$code=$writer->write('CoreMacros::includeTemplate(%node.word, %node.array? + $template->getParameters(), $_l->templates[%var])',$this->getCompiler()->getTemplateId());if($node->modifiers){return$writer->write('echo %modify(%raw->__toString(TRUE))',$code);}else{return$code.'->render()';}}function
...
macroAttr(MacroNode$node,$writer){return$writer->write('echo Html::el(NULL, %node.array)->attributes()');}function
macroOldAttr(MacroNode$node){return
Strings::replace($node->args.' ','#\)\s+#',')->');}function
macroDump(MacroNode$node,$writer){$args=$writer->formatArgs();return$writer->write('Debugger::barDump('.($node->args?"array(%var => $args)":'get_defined_vars()').', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))',$args);}function
...
instanceof
IFileTemplate){if(substr($destination,0,1)!=='/'&&substr($destination,1,1)!==':'){$destination=dirname($template->getFile()).'/'.$destination;}$tpl->setFile($destination);}}$tpl->setParameters($params);return$tpl;}static
function
...
function
install(LatteCompiler$compiler){$me=new
self($compiler);$me->addMacro('include',array($me,'macroInclude'));$me->addMacro('includeblock',array($me,'macroIncludeBlock'));$me->addMacro('extends',array($me,'macroExtends'));$me->addMacro('layout',array($me,'macroExtends'));$me->addMacro('block',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('define',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('snippet',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('ifset',array($me,'macroIfset'),'endif');$me->addMacro('widget',array($me,'macroControl'));$me->addMacro('control',array($me,'macroControl'));$me->addMacro('href',NULL,NULL,create_function('MacroNode $node, $writer','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me)).'], EXTR_REFS);
...
macroIncludeBlock(MacroNode$node,$writer){return$writer->write('CoreMacros::includeTemplate(%node.word, %node.array? + get_defined_vars(), $_l->templates[%var])->render()',$this->getCompiler()->getTemplateId());}function
...
macroIfCurrent(MacroNode$node,$writer){return$writer->write(($node->args?'try { $_presenter->link(%node.word, %node.array?); } catch (InvalidLinkException $e) {}':'').'; if ($_presenter->getLastCreatedRequestFlag("current")):');}function
...
instanceof
Control){foreach($control->getComponents(FALSE,'Control')as$child){if($child->isControlInvalid()){$child->snippetMode=TRUE;$child->render();$child->snippetMode=FALSE;}}}}}class
Tokenizer
extends
...
new
InvalidArgumentException('Template is not valid UTF-8 stream.');}$input=str_replace("\r\n","\n",$input);$this->input=$input;$this->output=array();$this->offset=0;$this->setSyntax($this->defaultSyntax);$this->setContext(self::CONTEXT_TEXT);$this->lastTag=$this->endTag=NULL;$this->xmlMode=(bool)preg_match('#^<\?xml\s#m',$input);while($this->offset<strlen($input)){$matches=$this->{"context".$this->context[0]}();if(!$matches){break;}elseif(!empty($matches['comment'])){$this->addToken(LatteToken::COMMENT,$matches[0]);}elseif(!empty($matches['macro'])){$token=$this->addToken(LatteToken::MACRO,$matches[0]);list($token->name,$token->value,$token->modifiers)=$this->parseMacro($matches['macro']);}$this->filter();}if($this->offset<strlen($input)){$this->addToken(LatteToken::TEXT,substr($this->input,$this->offset));}return$this->output;}private
function
contextText(){$matches=$this->match('~
			(?:(?<=\n|^)[ \t]*)?<(?P<closing>/?)(?P<tag>[a-z0-9:]+)|  ##  begin of HTML tag <tag </tag - ignores <!DOCTYPE
			<(?P<htmlcomment>!--)|     ##  begin of HTML comment <!--
			'.$this->macroRe.'         ##  macro
		~xsi');if(!empty($matches['htmlcomment'])){$this->addToken(LatteToken::TAG_BEGIN,$matches[0]);$this->setContext(self::CONTEXT_COMMENT);}elseif(!empty($matches['tag'])){$token=$this->addToken(LatteToken::TAG_BEGIN,$matches[0]);$token->name=$matches['tag'];$token->closing=(bool)$matches['closing'];$this->lastTag=$matches['closing'].strtolower($matches['tag']);$this->setContext(self::CONTEXT_TAG);}return$matches;}private
function
contextCData(){$matches=$this->match('~
			</(?P<tag>'.$this->lastTag.')(?![a-z0-9:])| ##  end HTML tag </tag
			'.$this->macroRe.'              ##  macro
		~xsi');if(!empty($matches['tag'])){$token=$this->addToken(LatteToken::TAG_BEGIN,$matches[0]);$token->name=$this->lastTag;$token->closing=TRUE;$this->lastTag='/'.$this->lastTag;$this->setContext(self::CONTEXT_TAG);}return$matches;}private
function
contextTag(){$matches=$this->match('~
			(?P<end>\ ?/?>)([ \t]*\n)?|  ##  end of HTML tag
			'.$this->macroRe.'|          ##  macro
			\s*(?P<attr>[^\s/>={]+)(?:\s*=\s*(?P<value>["\']|[^\s/>{]+))? ## begin of HTML attribute
		~xsi');if(!empty($matches['end'])){$this->addToken(LatteToken::TAG_END,$matches[0]);$this->setContext(!$this->xmlMode&&in_array($this->lastTag,array('script','style'))?self::CONTEXT_CDATA:self::CONTEXT_TEXT);}elseif(!empty($matches['attr'])){$token=$this->addToken(LatteToken::ATTRIBUTE,$matches[0]);$token->name=$matches['attr'];$token->value=isset($matches['value'])?$matches['value']:'';if($token->value==='"'||$token->value==="'"){if(Strings::startsWith($token->name,self::N_PREFIX)){$token->value='';if($m=$this->match('~(.*?)'.$matches['value'].'~xsi')){$token->value=$m[1];$token->text.=$m[0];}}else{$this->setContext(self::CONTEXT_ATTRIBUTE,$matches['value']);}}}return$matches;}private
function
...
contextComment(){$matches=$this->match('~
			...
			'.$this->macroRe.'         ##  macro
		~xsi');if(!empty($matches['htmlcomment'])){$this->addToken(LatteToken::TAG_END,$matches[0]);$this->setContext(self::CONTEXT_TEXT);}return$matches;}private
function
...
function
filter(){$token=end($this->output);if($token->type===LatteToken::MACRO&&$token->name==='/syntax'){$this->setSyntax($this->defaultSyntax);$token->type=LatteToken::COMMENT;}elseif($token->type===LatteToken::MACRO&&$token->name==='syntax'){$this->setSyntax($token->value);$token->type=LatteToken::COMMENT;}elseif($token->type===LatteToken::ATTRIBUTE&&$token->name==='n:syntax'){$this->setSyntax($token->value);$this->endTag='/'.$this->lastTag;$token->type=LatteToken::COMMENT;}elseif($token->type===LatteToken::TAG_END&&$this->lastTag===$this->endTag){$this->setSyntax($this->defaultSyntax);}elseif($token->type===LatteToken::MACRO&&$token->name==='contentType'){if(preg_match('#html|xml#',$token->value,$m)){$this->xmlMode=$m[0]==='xml';$this->setContext(self::CONTEXT_TEXT);}else{$this->setContext(self::CONTEXT_NONE);}}}}class
PhpWriter
extends
...
write($mask){$args=func_get_args();array_shift($args);$word=strpos($mask,'%node.word')===FALSE?NULL:$this->argsTokenizer->fetchWord();$me=$this;$mask=Strings::replace($mask,'#%escape(\(([^()]*+|(?1))+\))#',callback(create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me)).'], EXTR_REFS);
		...
		')));return
Strings::replace($mask,'#([,+]\s*)?%(node\.word|node\.array|node\.args|var|raw)(\?)?(\s*\+\s*)?()#',callback(create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me,'word'=>$word,'args'=>&$args)).'], EXTR_REFS);
...
extends
Object{const
TEXT='text',MACRO='macro',TAG_BEGIN='tag_begin',TAG_END='tag_end',ATTRIBUTE='attribute',COMMENT='comment';public$type;public$text;public$line;public$name;public$value;public$modifiers;public$closing;}class
RobotLoader
extends
AutoLoader{const
...
tryLoad($type){$type=ltrim(strtolower($type),'\\');$info=&$this->list[$type];if($this->autoRebuild&&empty($this->checked[$type])&&(is_array($info)?!is_file($info[0]):$info<self::RETRY_LIMIT)){$info=is_int($info)?$info+1:0;$this->checked[$type]=TRUE;if($this->rebuilt){$this->getCache()->save($this->getKey(),$this->list,array(Cache::CONSTS=>'Framework::REVISION'));}else{$this->rebuild();}}if(isset($info[0])){LimitedScope::load($info[0],TRUE);if($this->autoRebuild&&!class_exists($type,FALSE)&&!interface_exists($type,FALSE)&&(PHP_VERSION_ID<50400||!trait_exists($type,FALSE))){$info=0;$this->checked[$type]=TRUE;if($this->rebuilt){$this->getCache()->save($this->getKey(),$this->list,array(Cache::CONSTS=>'Framework::REVISION'));}else{$this->rebuild();}}self::$count++;}}function
...
as$pair){if(is_array($pair)){$this->files[$pair[0]]=$pair[1];}}foreach(array_unique($this->scanDirs)as$dir){$this->scanDirectory($dir);}$this->files=NULL;$dp=array(Cache::CONSTS=>'Framework::REVISION');return$this->list;}function
...
InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in ".$this->list[$lClass][0].".");if(PHP_VERSION_ID<50300){Debugger::_exceptionHandler($e);exit;}else{throw$e;}}$this->list[$lClass]=array($file,$time,$class);$this->files[$file]=$time;}private
function
...
as$item){if($item=realpath($item)){$disallow[$item]=TRUE;}}$iterator=Finder::findFiles(is_array($this->acceptFiles)?$this->acceptFiles:Strings::split($this->acceptFiles,'#[,\s]+#'))->filter(create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('disallow'=>&$disallow)).'], EXTR_REFS);
...
send(Mail$mail){$tmp=clone$mail;$tmp->setHeader('Subject',NULL);$tmp->setHeader('To',NULL);$parts=explode(Mail::EOL.Mail::EOL,$tmp->generateMessage(),2);Debugger::tryError();$args=array(str_replace(Mail::EOL,PHP_EOL,$mail->getEncodedHeader('To')),str_replace(Mail::EOL,PHP_EOL,$mail->getEncodedHeader('Subject')),str_replace(Mail::EOL,PHP_EOL,$parts[1]),str_replace(Mail::EOL,PHP_EOL,$parts[0]));if($this->commandArgs){$args[]=(string)$this->commandArgs;}$res=call_user_func_array('mail',$args);if(Debugger::catchError($e)){throw
new
...
getClassName(){return($tmp=Strings::match($this,'#>\s+([a-z0-9_\\\\]+)#i'))?$tmp[1]:NULL;}function
...
__construct(IUserStorage$storage,IDIContainer$context){$this->storage=$storage;$this->context=$context;}final
function
getStorage(){return$this->storage;}function
...
ob_get_clean();}catch(Exception$e){ob_end_clean();if($args&&$args[0]){throw$e;}else{Debugger::toStringException($e);}}}function
...
function
strip($s){return
Strings::replace($s,'#(</textarea|</pre|</script|^).*?(?=<textarea|<pre|<script|$)#si',callback(create_function('$m','
				return trim(preg_replace("#[ \\t\\r\\n]+#", " ", $m[0]));
			')));}static
function
indent($s,$level=1,$chars="\t"){if($level>=1){$s=Strings::replace($s,'#<(textarea|pre).*?</\\1#si',callback(create_function('$m','
				return strtr($m[0], " \\t\\r\\n", "\\x1F\\x1E\\x1D\\x1A");
...
function
dataStream($data,$type=NULL){if($type===NULL){$type=MimeTypeDetector::fromString($data,NULL);}return'data:'.($type?"$type;":'').'base64,'.base64_encode($data);}static
function
...
function
grep(array$arr,$pattern,$flags=0){Debugger::tryError();$res=preg_grep($pattern,$arr,$flags);if(Debugger::catchError($e)||preg_last_error()){throw
new
...
NRecursiveCallbackFilterIterator($iterator,create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('filters'=>$filters)).'], EXTR_REFS);
...
NCallbackFilterIterator($iterator,create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('groups'=>$groups)).'], EXTR_REFS);
...
new
InvalidArgumentException('Invalid date predicate format.');}list(,$operator,$date)=$matches;$operator=$operator?$operator:'=';}$date=DateTime53::from($date)->format('U');return$this->filter(create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('operator'=>$operator,'date'=>$date)).'], EXTR_REFS);
...
StaticClassException;}static
function
encode($value){Debugger::tryError();if(function_exists('ini_set')){$old=ini_set('display_errors',0);$json=json_encode($value);ini_set('display_errors',$old);}else{$json=json_encode($value);}if(Debugger::catchError($e)){throw
new
...
DateTime53($t);}else{$value=$t;}$hasValue=TRUE;}}if($inlineParser){if($hasValue){$this->addValue($result,$hasKey,$key,$value);}elseif($hasKey){$this->error();}}else{if($hasValue&&!$hasKey){if($result===NULL){$result=$value;}else{$this->error();}}elseif($hasKey){$this->addValue($result,$key!==NULL,$key,$hasValue?$value:NULL);}}return$result;}private
function
...
as$property){$properties[]=($property->documents?str_replace("\n","\n * ","/**\n".implode("\n",(array)$property->documents))."\n */\n":'').$property->visibility.' $'.$property->name.($property->value===NULL?'':' = '.PhpHelpers::dump($property->value)).";\n";}return
...
new
InvalidStateException("Callback '$textual' is not callable.");}Debugger::tryError();preg_match($pattern,'');if(Debugger::catchError($e)){throw
new
...
extends
Object{protected
static$validators=array('bool'=>'is_bool','boolean'=>'is_bool','int'=>'is_int','integer'=>'is_int','float'=>'is_float','number'=>NULL,'numeric'=>'Validators::isNumeric','numericint'=>'Validators::isNumericInt','string'=>'is_string','unicode'=>'Validators::isUnicode','array'=>'is_array','list'=>'Validators::isList','object'=>'is_object','resource'=>'is_resource','scalar'=>'is_scalar','callable'=>'Validators::isCallable','null'=>'is_null','email'=>'Validators::isEmail','url'=>'Validators::isUrl','none'=>'Validators::isNone','pattern'=>NULL,'alnum'=>'ctype_alnum','alpha'=>'ctype_alpha','digit'=>'ctype_digit','lower'=>'ctype_lower','upper'=>'ctype_upper','space'=>'ctype_space','xdigit'=>'ctype_xdigit');protected
static$counters=array('string'=>'strlen','unicode'=>'Strings::length','array'=>'count','list'=>'count','alnum'=>'strlen','alpha'=>'strlen','digit'=>'strlen','lower'=>'strlen','space'=>'strlen','upper'=>'strlen','xdigit'=>'strlen');static
function
