...
class googlePolyLine {
  ...
  # elimination under the DP Algorithm.
	...
	function dpEncode($points) {
		...
		if (count($points) > 2) {
			...
			while (count($stack) > 0) {
				...
				for ($i = $current[0] + 1; $i < $current[1]; $i++) { //go i from start to end
					$temp = $this->distance($points[$i], $points[$current[0]], $points[$current[1]]); //compute orthogonal distance start>i>end
					if ($temp > $maxDist) {
						$maxDist = $temp;
						$maxLoc = $i;
						if ($maxDist > $absMaxDist) { //keeping track of absMaxSize
							$absMaxDist = $maxDist;
						}
					}
				}
	...
	function distance($p0, $p1, $p2) { //orthogonal distance on sphere
