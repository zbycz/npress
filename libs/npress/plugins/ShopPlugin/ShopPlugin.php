...
class ShopPlugin extends Control{
	static $events = array('displayPageContent', 'renderAdminList',
	...
	function filterPageEditForm_create(AppForm $form){
		if ($this->page->getParent()->getMeta('.category') == 'shop'){
			$form->addSelect("id_parent_change", "kategorie",
	...
	public function renderList(){
		if($this->page->id == 8){
			...
			foreach($this->page->getChildNodes() as $r){
				$this->template->products = array_merge($this->template->products, $r->getChildNodes());
			}
		}
		else
