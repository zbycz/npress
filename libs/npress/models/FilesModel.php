...
class FilesModel extends Object {
	...
	public static function getGalleries($id_page, $addBlank=false){
		...
		foreach(self::getFiles($id_page) as $f){
			...
			if(!isset($galleries[$k])) $galleries[$k] = array();
			$galleries[$k][] = $f;
			$maxnum = max($maxnum, $k);
		}
	...
	public static function getFile($id){ //TODO cache
	 ...
	 * @return File
	 */
	public static function getFileByName($id_page, $filename){ //TODO cache
	...
	public static function getFileByOrigpath($origpath){ //TODO cache
			$r = dibi::fetch('SELECT * FROM pages_files WHERE %and',array(
					'origpath' => $origpath,
			));
			if($r) return self::createFileObject($r);
			return false;
	}
	...
	public static function edit($data){
		return dibi::query('UPDATE pages_files SET',$data,' WHERE id = %i', $data['id']);
	}
	...
	public static function createNew($id_page, $path, $options=''){
		...
		dibi::query('INSERT INTO pages_files', array(
				'id_page' => $id_page,
				'filename' => $pathinfo['filename'],
				'suffix' => isset($pathinfo['extension']) ? strtolower($pathinfo['extension']) : '',
				'ord' => $ord,
		));
	...
	public static function sortFilesBy($id_page, $orderBy){
		...
		$i = 0;
		foreach($result as $r){
			dibi::query("UPDATE pages_files SET ord = %i",$i++," WHERE id=%i",$r['id']);
		}
	}
